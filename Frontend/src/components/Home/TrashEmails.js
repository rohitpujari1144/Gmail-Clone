...
function TrashEmails() {
    ...
    function moveToInboxClick(email) {
        ...
        axios.put(`https://new-be.onrender.com/updateEmailInfo/${email._id}`, sendingEmailToInbox)
            .then((response) => {
    ...
    function emptyTrashNow() {
        if (trashEmails.length !== 0) {
            axios.delete(`https://new-be.onrender.com/deleteAllTrashEmails/${userEmail}`)
                .then((response) => {
