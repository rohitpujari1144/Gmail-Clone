...
// deleteing particular draft email from All Emails collection
app.delete('/deleteDraftEmail/:emailId', async (req, res) => {
    const client = await MongoClient.connect(dbUrl)
    try {
        const db = await client.db('Gmail_Clone')
        let removedDraftEmail = await db.collection('All Emails').deleteOne({ _id: mongodb.ObjectId(req.params.emailId) })
        res.status(200).send({ message: 'Draft Email deleted successfully', data: removedDraftEmail })
    }
    catch (error) {
        console.log(error);
        res.status(500).send({ message: 'Internal server error', error })
